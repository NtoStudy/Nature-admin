  ...
  setup(props) {
    ...
    const defaultOpeneds = computed(() => {
      ...
      $tree.recursionMachine(props.menuList, (menu) => {
        if (isArray(menu.children) && menu.children.length > 0 && menu.meta?.defaultOpen) {
    ...
    watch(route, (nVal) => {
      ...
      defaultActive.value = matchedMenusWithoutHidden[matchedMenusWithoutHidden.length - 1]?.path
    })
