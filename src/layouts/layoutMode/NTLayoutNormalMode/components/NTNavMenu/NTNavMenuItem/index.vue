...
<template>
  <template v-for="(item, index) in menuList" :key="item.path + index">
    <el-sub-menu
      v-if="item.children && hasNoneHiddenChildrenMenu(item.children) && item?.meta?.hidden !== true"
      :index="item.path"
    >
      <template #title>
    ...
    >
      <i class="menu__icon mr-[4px]" :class="item?.meta?.icon"> </i>
      <template #title>
        <div class="menu-link__wrapper" @click.prevent="handleRouterMenuLink(item.path)">
          <span>{{ item?.meta?.title }}</span>
        </div>
      </template>
    </el-menu-item>
