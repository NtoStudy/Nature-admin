...
-->
<template>
  <div class="nt-header clear-both" :class="{ 'nt-header--fixed': isHeaderFixed }">
    <el-row>
      ...
      <el-col :xs="18" :sm="12" :md="12" :lg="12" :xl="12" is-guttered>
        <div class="float-right">
          <el-menu mode="horizontal" :ellipsis="false" :collapse-transition="false">
            ...
            <el-sub-menu index="2">
              <template #title>
                <img src="@/assets/images/avatar.png" class="w-[40px] mr-[4px] rounded-[50%]" />
...
import { NO_AUTH_ROUTE_PATH } from '@/settings/config/router'
      ...
      ElMessageBox.confirm('确定退出系统吗?', '提示', { type: 'warning' })
        .then(async () => {
          userStore
            ...
            .then(() => {
              appStore.clear()
              router.replace(NO_AUTH_ROUTE_PATH)
            })
            .catch(() => {
              ElMessage.error({
                message: '退出登录失败！',
